...
class DisagreementSerializer(serializers.ModelSerializer):
    ...
    record_a_extra_data = serializers.JSONField(
        source="record_a.extra_data", read_only=True, default=dict
    )
    entry_b_extra_data = serializers.JSONField(
        source="entry_b.extra_data", read_only=True, default=dict
    )
